...
READ_ONLY_METHOD_PREFIXES = ("crm.company.list", "crm.company.get", "crm.company.fields", "crm.requisite.list", "crm.requisite.fields", "crm.activity.list", "crm.deal.list", "crm.company.contact.items.get", "user.current", "methods")
...
class BitrixClient:
    ...
    def _endpoint(self, method: str) -> str:
        return parse.urljoin(self.base, method)

    def call(self, method: str, params: dict[str, Any] | None = None) -> Any:
        params = params or {}
        payload = parse.urlencode(params, doseq=True).encode()
        last_error: Exception | None = None
        for attempt in range(self.max_retries + 1):
            try:
                req = request.Request(self._endpoint(method), data=payload)
    ...
    def batch(self, commands: dict[str, str]) -> dict[str, Any]:
        result = self.call("batch", {"cmd": commands})
...
def get_company_last_activity(client: BitrixClient, company_id: str) -> datetime | None:
    rows = client.paginated_list("crm.activity.list", {"filter": {"OWNER_TYPE_ID": 4, "OWNER_ID": company_id}, "order": {"LAST_UPDATED": "DESC"}, "select": ["LAST_UPDATED", "CREATED"], "limit": 1})
    if not rows:
        return None
    return parse_dt(rows[0].get("LAST_UPDATED") or rows[0].get("CREATED"))
...
def merge_fields_into_winner(winner: dict[str, Any], loser: dict[str, Any]) -> tuple[dict[str, Any], list[str]]:
    updates, conflicts = {}, []
...
def merge_communications(winner: dict[str, Any], loser: dict[str, Any]) -> dict[str, list[dict[str, Any]]]:
    merged = {}
...
def move_deals_to_winner(client: BitrixClient, winner_id: str, loser_id: str) -> int:
    deals = client.paginated_list("crm.deal.list", {"filter": {"COMPANY_ID": loser_id}, "select": ["ID"], "limit": 50})
    for deal in deals:
        client.call("crm.deal.update", {"id": deal["ID"], "fields": {"COMPANY_ID": winner_id}})
    return len(deals)


def move_contacts_to_winner(client: BitrixClient, winner_id: str, loser_id: str) -> int:
    contacts = client.call("crm.company.contact.items.get", {"id": loser_id}) or []
    moved = 0
    for contact in contacts:
        cid = contact.get("CONTACT_ID") or contact.get("CONTACT_ID".lower()) or contact.get("ID")
        if not cid:
            continue
...
def move_requisites_to_winner(client: BitrixClient, winner_id: str, loser_id: str, page_size: int) -> int:
    reqs = client.paginated_list("crm.requisite.list", {"filter": {"ENTITY_TYPE_ID": 4, "ENTITY_ID": loser_id}, "select": ["*"], "limit": page_size})
    moved = 0
    for req in reqs:
...
def build_merge_plan(groups: dict[str, list[dict[str, Any]]], allow_unknown: bool, allow_ogrn_conflict: bool, client: BitrixClient | None = None, page_size: int = 50) -> dict[str, Any]:
    plan_groups = []
    for inn, group in groups.items():
        ...
        if any(x["owner_group"] == "UNKNOWN_USER" for x in group) and winner["owner_group"] != "ACTIVE_MANAGER" and not allow_unknown:
            ...
            allowed = False
        fields_transfer, comm_transfer, conflicts = {}, {}, []
        for loser in losers:
            up, conf = merge_fields_into_winner(winner["company"], loser["company"])
            fields_transfer[loser["company"]["ID"]] = up
            comm_transfer[loser["company"]["ID"]] = merge_communications(winner["company"], loser["company"])
            conflicts.extend(conf)
        plan_groups.append({"inn": inn, "winner_id": winner["company"]["ID"], "winner_owner_group": winner["owner_group"], "loser_ids": [x["company"]["ID"] for x in losers], "ogrn_conflict": ogrn_conflict, "field_updates": fields_transfer, "communication_updates": comm_transfer, "conflicts": sorted(set(conflicts)), "warnings": warnings, "allowed_for_apply": allowed})
...
def apply_merge_plan(client: BitrixClient, plan: dict[str, Any], page_size: int, errors_path: Path) -> None:
    ...
    for group in plan["groups"]:
        ...
        for loser_id in group["loser_ids"]:
            ...
            comm = merge_communications(winner, loser)
            update_fields = {**fields, **comm}
            if update_fields:
                client.call("crm.company.update", {"id": winner_id, "fields": update_fields})
                winner.update(update_fields)
            move_deals_to_winner(client, winner_id, loser_id)
            move_contacts_to_winner(client, winner_id, loser_id)
            move_requisites_to_winner(client, winner_id, loser_id, page_size)
        processed += 1
...
def main() -> int:
    ...
    for company in companies:
        ...
        if identifiers["inn"]:
            with_inn += 1
        rows.append({"company": company, "identifiers": identifiers, "owner_group": owner_group, "last_activity": get_company_last_activity(client, cid)})
    ...
    plan["metadata"] = {"native_merge_method_available": any(m for m in methods if "merge" in m and "company" in m)}
    if args.dry_run:
