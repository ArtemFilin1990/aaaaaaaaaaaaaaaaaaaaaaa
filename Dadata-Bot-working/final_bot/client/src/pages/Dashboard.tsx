  ...
  useEffect(() => {
    if (config) {
      form.reset({
        ...
        checkoApiKey: config.checkoApiKey || "",
  ...
  return (
    <div className="min-h-screen bg-[#F2F2F7] pb-10">
      ...
      <header className="px-4 pt-14 pb-4 bg-[#F2F2F7]/80 backdrop-blur-md sticky top-0 z-50">
        <h1 className="text-[34px] font-bold tracking-tight text-black">Бот проверки контрагентов</h1>
      </header>
      ...
      <div className="px-4 space-y-8 max-w-2xl mx-auto">
            ...
            {!isConfigured && (
              <div className="ios-row py-2">
                <p className="text-[13px] text-amber-600 flex items-center gap-1.5">
                  ...
                  Заполните ключи и включите бота
                </p>
              </div>
            )}
          </div>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            <div className="ios-inset-group">
              ...
              <div className="ios-row">
                ...
                <input
                  type="password"
                  placeholder="Секретный ключ Dadata (опционально)"
                  className="ios-input"
                  {...form.register("dadataSecretKey")}
                />
              </div>
              <div className="ios-row">
                <Database className="w-5 h-5 text-[#FF9500] mr-3" />
                <input
                  type="password"
                  placeholder="API-ключ Checko (опционально)"
                  className="ios-input"
                  {...form.register("checkoApiKey")}
                />
              </div>
            </div>
        ...
        <p className="text-center text-[12px] text-zinc-400 font-medium">
          Dadata + Checko Telegram Bot v2.0
        </p>
      </div>
    </div>
  );
